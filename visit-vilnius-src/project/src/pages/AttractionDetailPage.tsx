...
import { ArrowLeft, MapPin, Clock, Star, Phone, Globe, Eye, Heart, Share2, Navigation } from 'lucide-react';
...
const AttractionDetailPage = () => {
  ...
  const backUrl = getCategoryUrl(locale, 'see');
  ...
  if (!item) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('detail.attraction.notFound')}</h2>
      ...
      <div className="bg-white border-b">
        <div className="container mx-auto px-4 py-4">
          <Link to={backUrl} className="inline-flex items-center space-x-2 text-blue-600 hover:text-blue-800 font-medium">
            ...
            <span>{t('category.attraction')}</span>
          </Link>
        </div>
      </div>
                ...
                <div className="space-y-4">
                  ...
                  {item.contact_info?.website && (
                    <div className="flex items-center space-x-3">
                      ...
                      <a href={item.contact_info.website} target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 hover:underline truncate" onClick={() => trackClick({ listingId: item.id, clickType: 'website', elementLabel: 'website_link' })}>
                        {item.contact_info.website}
                      </a>
                    </div>
                  )}
              ...
              <div className="space-y-3">
                {(item.latitude && item.longitude) ? (
                  <a
                    ...
                    onClick={() => trackClick({ listingId: item.id, clickType: 'map', elementLabel: 'map_button' })}
                  >
                ...
                {item.contact_info?.phone && (
                  <a
                    ...
                    onClick={() => trackClick({ listingId: item.id, clickType: 'phone', elementLabel: 'call_button' })}
                  >
