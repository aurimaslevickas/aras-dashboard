...
const ShopPage = () => {
  ...
  const section = useSectionTexts('shop');
  ...
  const filters = [
    { value: 'all', label: t('filter.allShops') },
    { value: 'souvenirs', label: t('filter.souvenirs') },
    { value: 'fashion', label: t('filter.fashion') },
    { value: 'books', label: t('filter.books') },
    { value: 'art', label: t('filter.art') },
    { value: 'craft', label: t('filter.craft') },
  ];
  ...
  const getCardUrl = (listing: Listing) => {
    const catUrl = getCategoryUrl(locale, 'shop');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-br from-green-600 via-green-700 to-teal-600 text-white py-10">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
              <ShoppingBag className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-5xl font-bold">{section.title || t('category.shop')}</h1>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-3xl font-bold text-gray-900">{t('filter.byCategory')}</h2>
            </div>
          </div>
          ...
          <div className="mb-8 pb-6 border-b border-gray-200">
            <p className="text-gray-600 text-lg">
              {t('filter.found')} <span className="font-bold text-green-600 text-xl">{filteredListings.length}</span> {t('filter.shops')}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">{t('filter.notFoundShops')}</h3>
            </div>
          ) : (
            <>
                ...
                {paginatedListings.map((listing, idx) => (
                  ...
                  >
                    <div className="relative h-56 overflow-hidden">
                      <ResponsiveImage
                        src={listing.image_url || 'https://images.pexels.com/photos/1488463/pexels-photo-1488463.jpeg?auto=compress&cs=tinysrgb&w=800'}
                      ...
                      {listing.features && listing.features.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {listing.features.slice(0, 3).map((feature) => (
                            <span key={feature} className="px-3 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-full">
                              {feature}
                            </span>
                          ))}
                        </div>
