...
import { MapPin, Wine, Filter } from 'lucide-react';
...
const BarPage = () => {
  ...
  const filters = [
    { value: 'all', label: t('filter.allBars') },
    { value: 'cocktail', label: t('filter.cocktailBars') },
    { value: 'craft_beer', label: t('filter.craftBeer') },
    { value: 'wine', label: t('filter.wineBars') },
    { value: 'sports', label: t('filter.sportsBars') },
    { value: 'rooftop', label: t('filter.rooftopBars') },
    { value: 'live_music', label: t('filter.liveMusic') },
  ];
  ...
  const getCardUrl = (listing: Listing) => {
    const catUrl = getCategoryUrl(locale, 'bar');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-br from-slate-800 via-slate-700 to-zinc-700 text-white py-10">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
              <Wine className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-5xl font-bold">{section.title || t('category.bar')}</h1>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-3xl font-bold text-gray-900">{t('filter.byType')}</h2>
            </div>
          </div>
          ...
          <div className="mb-8 pb-6 border-b border-gray-200">
            <p className="text-gray-600 text-lg">
              {t('filter.found')} <span className="font-bold text-violet-600 text-xl">{filteredListings.length}</span> {t('filter.bars')}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">{t('filter.notFoundBars')}</h3>
            </div>
          ) : (
            <>
                ...
                {paginatedListings.map((listing, idx) => (
                  ...
                  >
                    <div className="relative h-56 overflow-hidden">
                      <ResponsiveImage
                        src={listing.image_url || 'https://images.pexels.com/photos/941864/pexels-photo-941864.jpeg?auto=compress&cs=tinysrgb&w=800'}
                      ...
                      {listing.price_range && (
                        <div className="absolute bottom-4 right-4 px-3 py-1.5 bg-slate-600 text-white text-xs font-bold rounded-full shadow-lg">
                          {listing.price_range}
                        </div>
                      )}
                    </div>
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-gray-900 group-hover:text-violet-600 transition-colors line-clamp-1 mb-3">
                      ...
                      {listing.features && listing.features.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {listing.features.slice(0, 3).map((feature) => (
                            <span key={feature} className="px-3 py-1 bg-slate-50 text-slate-700 text-xs font-medium rounded-full">
                              {feature}
                            </span>
                          ))}
                        </div>
              ...
              {!mobileExpanded && filteredListings.length > MOBILE_INITIAL && (
                <div className="flex justify-center md:hidden mb-8">
                  <button
                    ...
                    className="px-8 py-3 bg-violet-600 text-white font-semibold rounded-xl hover:bg-violet-700 transition-colors shadow-lg"
                  >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-lg font-semibold transition-colors ${
                          currentPage === page ? 'bg-violet-600 text-white' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'
                        }`}
                      >
                        {page}
                      </button>
