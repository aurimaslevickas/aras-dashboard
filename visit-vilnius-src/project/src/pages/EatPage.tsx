...
import { MapPin, Utensils, Filter, Award } from 'lucide-react';
...
const EatPage = () => {
  ...
  const filters = [
    { value: 'all', label: t('filter.allRestaurants') },
    { value: 'michelin', label: 'Michelin' },
    { value: 'cafe', label: t('filter.cafe') },
    { value: 'fine_dining', label: t('filter.fineDining') },
    { value: 'casual', label: t('filter.casual') },
    { value: 'vegetarian', label: t('filter.vegetarian') },
  ];
  ...
  const fetchListings = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .order('michelin_nominated', { ascending: false })
  ...
  const getCardUrl = (listing: Listing) => {
    const catUrl = getCategoryUrl(locale, 'eat');
    const slug = getLocalizedSlug(listing);
    return `${catUrl}/${slug}`;
  };

  const filteredListings = listings.filter((listing) => {
    if (selectedFilter === 'michelin') return listing.michelin_stars > 0 || listing.michelin_nominated;
    if (selectedFilter === 'cafe') return listing.features?.includes('cafe');
    if (selectedFilter !== 'all') return listing.features?.includes(selectedFilter);
    return true;
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-br from-orange-600 via-red-600 to-pink-600 text-white py-10">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
              <Utensils className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-5xl font-bold">{section.title || t('category.restaurant')}</h1>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-3xl font-bold text-gray-900">{t('filter.findRestaurant')}</h2>
            </div>
          </div>
          ...
          <div className="mb-8 pb-6 border-b border-gray-200">
            <p className="text-gray-600 text-lg">
              {t('filter.found')} <span className="font-bold text-orange-600 text-xl">{filteredListings.length}</span> {t('filter.restaurants')}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">{t('filter.notFoundRestaurants')}</h3>
            </div>
          ) : (
            <>
                ...
                {paginatedListings.map((listing, idx) => (
                  ...
                  >
                    <div className="relative h-56 overflow-hidden">
                      <ResponsiveImage
                        src={listing.image_url || 'https://images.pexels.com/photos/262978/pexels-photo-262978.jpeg?auto=compress&cs=tinysrgb&w=800'}
                      ...
                      {listing.michelin_stars > 0 && (
                        <div className="absolute top-4 left-4 px-3 py-1.5 bg-red-600 text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1">
                          <Award className="w-3 h-3" />
                          {listing.michelin_stars} Michelin {listing.michelin_stars === 1 ? 'Star' : 'Stars'}
                        </div>
                      )}
                      {!listing.michelin_stars && listing.michelin_nominated && (
                        <div className="absolute top-4 left-4 px-3 py-1.5 bg-yellow-600 text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1">
                          <Award className="w-3 h-3" />
                          Michelin Nominated
                        </div>
                      )}
                      {listing.badge && !listing.michelin_stars && !listing.michelin_nominated && (
