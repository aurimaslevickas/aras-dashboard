...
const SeePage = () => {
  ...
  const filters = [
    { value: 'all', label: t('filter.allAttractions'), tags: [] },
    { value: 'museum', label: t('filter.museums'), tags: ['muziejus', 'galerija', 'paroda'] },
    { value: 'church', label: t('filter.churches'), tags: ['bažnyčia', 'katedra', 'varpinė'] },
    { value: 'viewpoint', label: t('filter.viewpoints'), tags: ['apžvalgos', 'bokštas', 'panorama'] },
    { value: 'park', label: t('filter.parks'), tags: ['parkas', 'gamta', 'miškas', 'žaluma'] },
    { value: 'historical', label: t('filter.historical'), tags: ['istorija', 'paveldas', 'pilis', 'senamiestis', 'architektūra'] },
  ];
  ...
  const getCardUrl = (listing: Listing) => {
    const catUrl = getCategoryUrl(locale, 'see');
  ...
  const filteredListings = listings.filter((listing) => {
    ...
    if (!activeFilter || activeFilter.tags.length === 0) return true;
    return activeFilter.tags.some((tag) => listing.features?.includes(tag));
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-br from-blue-600 via-sky-600 to-cyan-600 text-white py-10">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center">
              <Eye className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-5xl font-bold">{section.title || t('category.sightseeing')}</h1>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-3xl font-bold text-gray-900">{t('filter.findAttraction')}</h2>
            </div>
          </div>
          ...
          <div className="mb-8 pb-6 border-b border-gray-200">
            <p className="text-gray-600 text-lg">
              {t('filter.found')} <span className="font-bold text-blue-600 text-xl">{filteredListings.length}</span> {t('filter.attractions')}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">{t('filter.notFoundAttractions')}</h3>
            </div>
          ) : (
            <>
                ...
                {paginatedListings.map((listing, idx) => (
                  ...
                  >
                    <div className="relative h-56 overflow-hidden">
                      <ResponsiveImage
                        src={listing.image_url || 'https://images.pexels.com/photos/208723/pexels-photo-208723.jpeg?auto=compress&cs=tinysrgb&w=800'}
                      ...
                      {listing.badge && (
                        <div className="absolute top-4 left-4 px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-full shadow-lg">
                          {['featured', 'free_entry', 'must_try', 'new', 'popular'].includes(listing.badge)
                            ? t(`common.${listing.badge}`)
                            : listing.badge}
                        </div>
                      )}
                      {listing.price_range && (
                        <div className="absolute bottom-4 right-4 px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-full shadow-lg">
                          {listing.price_range}
                        </div>
                      )}
                    </div>
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2 leading-tight mb-3">
                      ...
                      {listing.features && listing.features.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {listing.features.slice(0, 3).map((feature) => (
                            <span key={feature} className="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-full">
                              {feature}
                            </span>
                          ))}
                        </div>
